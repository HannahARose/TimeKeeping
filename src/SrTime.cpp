...
void setupArgParser(argparse::ArgumentParser& parser, int argc, char* argv[])
{
    parser.add_description(
        "SrTime - Calculate time deviation between NIST "
...
int main(int argc, char* argv[])
{
    ...
    argparse::ArgumentParser parser(
        "SrTime",
        std::format(
            "v{}.{}", Timekeeping_VERSION_MAJOR, Timekeeping_VERSION_MINOR),
    ...
    CsvGroupMetadata si_freq_metadata(
        si_freq_path,
        si_freq_template,
        {},
        "",
        ...
        {"Time", "Si_Freq"},
        -1);
    CsvTimeGroup si_freq_files(
        si_freq_metadata, CsvTimeFormat::oneColStandard, false);
    ...
    CsvGroupMetadata phase_freq_metadata(
        ...
        {"Day",
         ...
         "H_Freq",
         "Z_Freq"});
    CsvTimeGroup phase_freq_files(
        phase_freq_metadata, CsvTimeFormat::twoColShort, false);

    quad si_offset = get_si_offset(config);
    std::cout << "Si3 Frequency Offset: " << si_offset << " Hz" << std::endl;

    long si_division = config["Si3_Division"].as_int64();
    ...
    date_time epoch_time = parseTime(
        TimeFormat::isoExtended, config["Epoch_Time"].as_string().c_str());
    std::cout << "Epoch Time: "
              << boost::posix_time::to_iso_extended_string(epoch_time)
              << std::endl;

    date_time start_time = parseTime(
        TimeFormat::isoExtended, config["Start_Time"].as_string().c_str());
    std::cout << "Start Time: "
              << boost::posix_time::to_iso_extended_string(start_time)
              << std::endl;
    date_time end_time = parseTime(
        TimeFormat::isoExtended, config["End_Time"].as_string().c_str());
    std::cout << "End Time: "
              << boost::posix_time::to_iso_extended_string(end_time)
              << std::endl;
    ...
    time_delt gap_limit = boost::posix_time::seconds(long(5 * time_step))
                        + boost::posix_time::microseconds(long(
                            (5 * time_step - long(2 * time_step)) * 1e6));

    // Copy config to output file
    config["Tool"] = "SrTime";
    config["Version"] = std::format(
        "v{}.{}.{}",
        Timekeeping_VERSION_MAJOR,
        Timekeeping_VERSION_MINOR,
        Timekeeping_VERSION_PATCH);
    config["Git_Commit"] = GIT_COMMIT_HASH;
    config["Git_Branch"] = GIT_BRANCH_NAME;
    ...
    while (current_time <= end_time)
    {
        ...
        quad time_deviation
            = (data_phase - acc_phase - epoch_data_phase) / h_freq;
        ...
        if (interval_count % 600 == 0)
        {
            ...
            progress_bar.updateProgress(
                progress.total_microseconds(),
                totaltime.total_microseconds(),
                msg.str());
        }
        ...
        {
            std::cerr << std::endl;
            std::cerr << "Warning: Large gap detected in data files."
                      << std::endl;
            std::cerr << "Gap: "
                      << (new_data_phase - data_phase) / new_data_freq << " s"
                      << std::endl;
            quad gap_size = (new_data_phase - data_phase) / new_data_freq;
            long gap_interval = long(round(gap_size / time_step));
            std::cerr << "Estimated " << gap_interval - 1
                      << " data points missing." << std::endl;
