...
def update(frame):
    ...
    txtimes = data['txtime'].values
    datetimes = pd.to_datetime(txtimes, unit='s')
    deltas = txtimes[1:] - txtimes[:-1]
    rate = 1/np.mean(deltas)

    duration = txtimes.max() - txtimes.min()
    ...
    sigoffsets = data['sigoffset'].values
    ...
    adjusted_no_out_offsets, adjusted_no_out_times = remove_outliers(adjusted_offset, times=txtimes, level=5, degree=3)
    ...
    fit = linregress(txtimes, offsets)
 
    offsets = offsets - (fit.intercept + fit.slope * txtimes)
